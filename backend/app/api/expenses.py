...
def list_expenses(db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    stmt = (
        ...
        .options(selectinload(Expense.approvals), selectinload(Expense.attachments), selectinload(Expense.quotation_options), selectinload(Expense.quotation_votes))
    ...
    if expense_ids:
        events = db.scalars(select(ApprovalStepEvent).where(
            ...
            ApprovalStepEvent.expense_id, ApprovalStepEvent.occurred_at.desc(),
...
def expense_dashboard(db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    open_statuses = [ExpenseStatus.QUOTATION_VOTING, ExpenseStatus.SUBMITTED,
                     ExpenseStatus.PENDING_APPROVAL, ExpenseStatus.APPROVED,
                     ExpenseStatus.NEEDS_REVISION]
    in_process = db.scalar(select(func.count(Expense.id)).where(Expense.status.in_(open_statuses))) or 0
    closed_24h = db.scalar(select(func.count(Expense.id)).where(
        Expense.status == ExpenseStatus.CLOSED, Expense.closed_at >= now_utc - timedelta(hours=24))) or 0
    pending_ids = set()
    if user.role != UserRole.ADMIN and user.can_approve:
        pending_ids.update(db.scalars(select(Approval.expense_id).where(
            ...
            Approval.status == ApprovalStatus.PENDING)).all())
    ...
    month_rows = db.execute(select(Expense.status, func.count(Expense.id)).where(
        Expense.created_at >= period_start_utc).group_by(Expense.status)).all()
    ...
    month_amount = db.scalar(select(func.coalesce(func.sum(Expense.amount), 0)).where(
        ...
        Expense.status.in_([ExpenseStatus.APPROVED, ExpenseStatus.CLOSED]))) or 0
    return {
        ...
        'pending_items': [{
            'request_id': item.request_id, 'display_id': item.display_id,
            'title': item.title, 'urgency': item.urgency,
            'status': item.status.value, 'created_at': _as_utc(item.created_at),
...
def create_expense(
    ...
    db.flush()
    if payload.request_type == 'MULTI_QUOTE':
        db.add_all([QuotationOption(expense_id=expense.id, option_number=index,
            supplier=option.supplier, amount=option.amount,
            item_url=str(option.item_url) if option.item_url else None, notes=option.notes)
    ...
    if payload.request_type == 'MULTI_QUOTE':
        voters = list(db.scalars(select(User).where(
            User.active.is_(True), User.can_approve.is_(True), User.role != UserRole.ADMIN)).all())
        for voter in voters:
    ...
    stmt = select(Expense).where(Expense.id == expense.id).options(selectinload(Expense.approvals), selectinload(Expense.attachments), selectinload(Expense.quotation_options), selectinload(Expense.quotation_votes))
...
def get_email_quotation_vote(token: str, db: Session = Depends(get_db)):
    ...
    expense = db.scalar(select(Expense).where(Expense.id == invitation.expense_id).options(
        selectinload(Expense.quotation_options), selectinload(Expense.quotation_votes),
    ...
    current = next((vote.quotation_option_id for vote in expense.quotation_votes
                    if vote.voter_user_id == invitation.voter_user_id), None)
    return {'kind': 'QUOTATION_VOTE', 'token': token, 'current_option_id': current,
            'expense': {'display_id': expense.display_id, 'title': expense.title,
                        'description': expense.description, 'urgency': expense.urgency,
                        'options': [{'id': option.id, 'option_number': option.option_number,
                                     'supplier': option.supplier, 'amount': str(option.amount),
                                     'item_url': option.item_url, 'notes': option.notes,
...
def decide_email_quotation_vote(token: str, payload: QuotationVoteRequest, db: Session = Depends(get_db)):
    invitation = db.scalar(select(QuotationVotingInvitation).where(
        QuotationVotingInvitation.token == token))
    if not invitation:
        raise HTTPException(status_code=404, detail='Invitación de votación no encontrada')
    ...
    return {'status': expense.status.value, 'display_id': expense.display_id,
            'quotation_option_id': payload.quotation_option_id}


@router.post('/{request_id}/quotation-vote', response_model=ExpenseOut)
def vote_quotation(request_id: str, payload: QuotationVoteRequest, db: Session = Depends(get_db), user: User = Depends(require_permission('can_approve'))):
    ...
    expense = db.scalar(select(Expense).where(or_(Expense.request_id == request_id, Expense.display_id == request_id))
        .options(selectinload(Expense.quotation_options), selectinload(Expense.quotation_votes), selectinload(Expense.approvals), selectinload(Expense.attachments)))
    if not expense:
        raise HTTPException(status_code=404, detail='Solicitud no encontrada')
    if expense.status != ExpenseStatus.QUOTATION_VOTING:
        raise HTTPException(status_code=409, detail='La votación de cotizaciones ya no está abierta')
    supported_option_ids = set(db.scalars(select(ExpenseAttachment.quotation_option_id).where(
        ExpenseAttachment.expense_id == expense.id,
        ExpenseAttachment.quotation_option_id.is_not(None))).all())
    unsupported = [item.option_number for item in expense.quotation_options
                   if not item.item_url and item.id not in supported_option_ids]
    if unsupported:
        raise HTTPException(status_code=409, detail=f'Falta soporte en las opciones: {", ".join(map(str, unsupported))}')
    option = next((item for item in expense.quotation_options if item.id == payload.quotation_option_id), None)
    if not option:
        raise HTTPException(status_code=422, detail='La cotización no pertenece a esta solicitud')
    vote = next((item for item in expense.quotation_votes if item.voter_user_id == user.id), None)
    previous = vote.quotation_option_id if vote else None
    if vote:
        vote.quotation_option_id = option.id
    else:
        vote = QuotationVote(expense_id=expense.id, quotation_option_id=option.id,
            voter_user_id=user.id, voter_email=user.email, voter_role=user.title)
        db.add(vote)
    db.add(QuotationVoteEvent(expense_id=expense.id, flow_id=expense.flow_id,
        voter_user_id=user.id, voter_email=user.email, voter_role=user.title,
        previous_option_id=previous, selected_option_id=option.id))
    db.commit()
    eligible_count = db.scalar(select(func.count(User.id)).where(
        User.active.is_(True), User.can_approve.is_(True), User.role != UserRole.ADMIN)) or 0
    votes = list(db.scalars(select(QuotationVote).where(QuotationVote.expense_id == expense.id)).all())
    if eligible_count and len(votes) >= eligible_count:
        counts = {}
        for item in votes: counts[item.quotation_option_id] = counts.get(item.quotation_option_id, 0) + 1
...
async def upload_quotation_option_attachment(
    ...
    option = db.scalar(select(QuotationOption).where(
        QuotationOption.id == option_id, QuotationOption.expense_id == expense.id))
    if not option:
        raise HTTPException(status_code=404, detail='Cotización no encontrada')
    if file.content_type not in ALLOWED_TYPES:
        raise HTTPException(status_code=415, detail='Solo se permiten archivos PDF, JPG, PNG o WEBP')
    ...
    attachment = ExpenseAttachment(
        expense_id=expense.id, quotation_option_id=option.id,
        original_name=safe_original, stored_name=stored_name,
        content_type=file.content_type, size=len(content), document_type='QUOTATION_OPTION')
    db.add(attachment); db.commit(); db.refresh(attachment)
    return attachment
