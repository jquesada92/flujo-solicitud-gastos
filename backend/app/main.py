...
from app.api import approvals, areas, audit, auth, expenses, rules, users
...
def seed_rules() -> None:
    with SessionLocal() as db:
        ...
        # Legacy rules tied to fixed email addresses remain disabled.
        db.execute(update(ApprovalRule).values(active=False))
        real_policy = db.scalar(
            select(ApprovalPolicy.id).where(~ApprovalPolicy.name.like('[PRUEBA]%')).limit(1)
        )
        if not real_policy:
            ...
            state = {
                ...
                'max_amount': None,
                'approval_mode': 'ALL',
                'approver_profile_codes': policy.approver_profile_codes,
                'active': True,
            }
            if admin:
                db.add(ApprovalPolicyChangeEvent(
                    event_type='POLICY_CREATED', policy_id=policy.id,
                    policy_name=policy.name, actor_user_id=admin.id,
                    actor_email='Sistema', changed_fields=list(state),
                    before_state=None, after_state=state,
                ))
            for demo_policy in db.scalars(select(ApprovalPolicy).where(
                ApprovalPolicy.name.like('[PRUEBA]%'),
...
def migrate_schema() -> None:
    """Idempotent compatibility migration for the existing MVP database.

    Property-domain tables and columns are intentionally NOT created here.
    Their destructive removal lives in a separate, backup-gated migration file.
    """
    ...
    with engine.connect() as connection:
        ...
        if 'title' not in user_columns:
            ...
            connection.execute(text(
                "UPDATE users SET title = CASE "
                "WHEN role='ADMIN' THEN 'ADMIN_SISTEMA' "
                "WHEN role='APPROVER' THEN 'VOCERO' "
                "WHEN role='REQUESTER' THEN 'ADMINISTRADORA' "
        ...
        if 'identity_document' not in user_columns:
            ...
            connection.execute(text(
                'CREATE UNIQUE INDEX IF NOT EXISTS ix_users_identity_document '
                'ON users (identity_document) WHERE identity_document IS NOT NULL'
            ))
        connection.execute(text(
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_users_identity_document_normalized "
            "ON users (upper(trim(identity_document))) "
            "WHERE identity_document IS NOT NULL AND trim(identity_document) <> ''"
        ))
        if 'analytics_id' not in user_columns:
            connection.execute(text('ALTER TABLE users ADD COLUMN analytics_id VARCHAR(64)'))
            connection.execute(text(
                'CREATE UNIQUE INDEX IF NOT EXISTS ix_users_analytics_id '
                'ON users (analytics_id) WHERE analytics_id IS NOT NULL'
            ))
        ...
        connection.execute(text(
            "UPDATE access_profiles SET name='Administrador' "
            "WHERE code='ADMINISTRADORA' AND name='Administradora'"
        ))
        connection.execute(text(
            "UPDATE access_profiles SET name='Vocal' WHERE code='VOCERO' AND name='Vocero'"
        ))

        if 'has_user_limit' not in profile_columns:
            connection.execute(text(
                'ALTER TABLE access_profiles ADD COLUMN has_user_limit BOOLEAN NOT NULL DEFAULT FALSE'
            ))
        if 'max_users' not in profile_columns:
            connection.execute(text('ALTER TABLE access_profiles ADD COLUMN max_users INTEGER'))
        connection.execute(text(
            "UPDATE access_profiles SET has_user_limit=TRUE, max_users=1 "
            "WHERE code IN ('PRESIDENTE','VICEPRESIDENTE','TESORERO') AND has_user_limit=FALSE"
        ))
        connection.execute(text('''
        ...
        '''))
        connection.execute(text(
            "UPDATE users SET can_request=TRUE, can_approve=TRUE, can_view=TRUE, can_configure=TRUE WHERE role='ADMIN'"
        ))
        ...
        if 'flow_id' not in approval_columns:
            ...
            connection.execute(text(
                'UPDATE approvals SET flow_id = expenses.flow_id '
                'FROM expenses WHERE approvals.expense_id = expenses.id'
            ))
            connection.execute(text('ALTER TABLE approvals ALTER COLUMN flow_id SET NOT NULL'))
        if 'created_at' not in approval_columns:
            connection.execute(text(
                'ALTER TABLE approvals ADD COLUMN created_at TIMESTAMPTZ NOT NULL DEFAULT now()'
            ))
        ...
        if 'urgency' not in columns:
            connection.execute(text(
                "ALTER TABLE expenses ADD COLUMN urgency VARCHAR(20) NOT NULL DEFAULT 'NORMAL'"
            ))
        ...
        if 'requester_analytics_id' not in columns:
            ...
            connection.execute(text(
                'CREATE INDEX IF NOT EXISTS ix_expenses_requester_analytics_id ON expenses (requester_analytics_id)'
            ))
        ...
        if 'request_type' not in columns:
            connection.execute(text(
                "ALTER TABLE expenses ADD COLUMN request_type VARCHAR(20) NOT NULL DEFAULT 'SIMPLE'"
            ))
        ...
        groups = connection.execute(text(
            'SELECT DISTINCT expense_type, EXTRACT(YEAR FROM created_at)::int AS year FROM expenses'
        )).mappings().all()
        for group in groups:
            area_code, year = group['expense_type'], group['year']
            rows = connection.execute(
                text(
                    'SELECT id FROM expenses WHERE expense_type=:area_code '
                    'AND EXTRACT(YEAR FROM created_at)=:year ORDER BY id'
                ),
                {'area_code': area_code, 'year': year},
            ).mappings().all()
            prefix = area_code[:3].upper().ljust(3, 'X')[:3]
            for sequence, row in enumerate(rows, 1):
                connection.execute(
                    text('UPDATE expenses SET display_id=:display_id WHERE id=:id'),
                    {'display_id': f'{prefix}-{year}-{sequence:011d}', 'id': row['id']},
                )
            counter_key = f'{area_code}:{year}'
            connection.execute(text('''
                INSERT INTO category_counters (category,last_value)
                VALUES (:area_key,:value)
                ON CONFLICT (category) DO UPDATE
                SET last_value=GREATEST(category_counters.last_value, EXCLUDED.last_value)
            '''), {'area_key': counter_key, 'value': len(rows)})
        ...
        attachment_columns = {
            column['name'] for column in inspect(engine).get_columns('expense_attachments')
        }
        if 'document_type' not in attachment_columns:
            connection.execute(text(
                "ALTER TABLE expense_attachments ADD COLUMN document_type VARCHAR(40) NOT NULL DEFAULT 'QUOTATION'"
            ))
        if 'quotation_option_id' not in attachment_columns:
            connection.execute(text(
                'ALTER TABLE expense_attachments ADD COLUMN quotation_option_id INTEGER '
                'REFERENCES quotation_options(id) ON DELETE CASCADE'
            ))
            connection.execute(text(
                'CREATE INDEX IF NOT EXISTS ix_expense_attachments_quotation_option_id '
                'ON expense_attachments (quotation_option_id)'
            ))
        if 'approval_mode' not in approval_columns:
            connection.execute(text(
                "ALTER TABLE approvals ADD COLUMN approval_mode VARCHAR(20) NOT NULL DEFAULT 'SEQUENTIAL'"
            ))
...
def backfill_analytics_ids() -> None:
    with SessionLocal() as db:
        ...
        for expense in db.scalars(
            select(Expense).where(Expense.requester_analytics_id.is_(None))
        ).all():
...
def seed_access_profiles() -> None:
    ...
    with SessionLocal() as db:
        ...
        db.add_all(
            ...
            for code, name, req, approve, view, configure in defaults
            if code not in existing
        )
        db.flush()
        profiles = {
            profile.code: profile for profile in db.scalars(select(AccessProfile)).all()
        }
        for user in db.scalars(select(User).where(User.role != UserRole.ADMIN)).all():
            ...
            if profile:
                ...
                user.role = (
                    UserRole.APPROVER if profile.can_approve
                    else UserRole.REQUESTER if profile.can_request
