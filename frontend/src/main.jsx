...
function EmailActionPage({ kind, token }) {
  ...
  if(kind === "approval" && data.status !== "PENDING") return <main className="single"><section className="card email-action-card"><p className="eyebrow">RESPUESTA YA REGISTRADA</p><h1>{expense.title}</h1><div className="notice success">Este enlace ya fue respondido. Tu respuesta registrada es: {statusName(data.status)}.</div><div className="email-expense-summary"><div><span>Solicitud</span><strong>{expense.display_id}</strong></div><div><span>Respuesta</span><strong>{statusName(data.status)}</strong></div></div></section></main>;
