...
function ChangePassword({ user, onChanged }) {
  const [form,setForm]=useState({current_password:'',new_password:'',confirmation:''}),[saving,setSaving]=useState(false),[error,setError]=useState('')
  const submit=async e=>{e.preventDefault();setError('');if(form.new_password!==form.confirmation){setError('Las contraseñas nuevas no coinciden.');return}setSaving(true);try{const result=await api('/api/auth/change-password',{method:'POST',body:JSON.stringify({current_password:form.current_password,new_password:form.new_password})});localStorage.setItem('access_token',result.access_token);onChanged(result.user)}catch(err){setError(err.message)}finally{setSaving(false)}}
...
function ClosurePanel({ expense, onDone, onCancel }) {
  const [invoice,setInvoice]=useState(null),[notes,setNotes]=useState(''),[saving,setSaving]=useState(false),[error,setError]=useState('')
  const submit=async e=>{e.preventDefault();setSaving(true);setError('');try{const data=new FormData();data.append('invoice',invoice);if(notes)data.append('notes',notes);await api(`/api/expenses/${expense.request_id}/close`,{method:'POST',body:data});onDone()}catch(err){setError(err.message)}finally{setSaving(false)}}
...
function Invoices({ categoryOptions=expenseTypes }) {
  ...
  const clear=()=>{setSearch('');setCategory('');setLoading(true);setError('');api('/api/expenses/invoices').then(setItems).catch(e=>setError(e.message)).finally(()=>setLoading(false))}
  const download=item=>downloadAttachment({id:item.attachment_id,original_name:item.original_name}).catch(e=>setError(e.message))
  return <section className="card"><div className="card-heading"><div><p className="eyebrow">DOCUMENTOS FINALES</p><h2>Facturas</h2></div></div><form className="table-filters" onSubmit={submit}><label>Buscar<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Factura, ID, proveedor, solicitante..." /></label><label>Categoría<select value={category} onChange={e=>setCategory(e.target.value)}><option value="">Todas</option>{categoryOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><button className="primary" disabled={loading}>{loading?'Buscando...':'Buscar'}</button>{(search||category)&&<button type="button" className="secondary" onClick={clear}>Limpiar</button>}<span className="filter-count">{items.length} factura{items.length===1?'':'s'}</span></form>{error?<div className="notice error">{error}</div>:loading?<p className="muted">Cargando facturas...</p>:items.length===0?<p className="muted">No se encontraron facturas.</p>:<div className="table-wrap"><table><thead><tr><th>Factura</th><th>Solicitud</th><th>Proveedor</th><th>Categoría</th><th>Monto</th><th>Solicitante</th><th>Cierre</th><th>Acción</th></tr></thead><tbody>{items.map(x=><tr key={x.attachment_id}><td><strong>{x.original_name}</strong><span className="subtext">{(x.size/1024/1024).toFixed(2)} MB · {new Date(x.uploaded_at).toLocaleString()}</span></td><td><strong>{x.display_id}</strong><span className="subtext">{x.title}</span></td><td>{x.supplier}</td><td>{categoryOptions.find(([code])=>code===x.expense_type)?.[1]||x.expense_type}<span className="subtext">{subcategoryName(x.expense_subcategory)}</span></td><td>${Number(x.amount).toLocaleString(undefined,{minimumFractionDigits:2})}</td><td>{x.requested_by}</td><td>{x.closed_at?new Date(x.closed_at).toLocaleString():'—'}<span className="subtext">{x.closed_by||''}</span></td><td><button className="primary nowrap" onClick={()=>download(x)}>Descargar</button></td></tr>)}</tbody></table></div>}</section>
}

function Users() {
  const blank={name:'',email:'',role:'REQUESTER'}; const [form,setForm]=useState(blank); const [users,setUsers]=useState([]); const [message,setMessage]=useState(null)
  const load=()=>api('/api/users').then(setUsers).catch(e=>setMessage({type:'error',text:e.message})); useEffect(load,[])
  const create=async e=>{e.preventDefault();try{await api('/api/users',{method:'POST',body:JSON.stringify(form)});setForm(blank);setMessage({type:'success',text:'Usuario registrado. Se envió una contraseña temporal a su correo.'});load()}catch(err){setMessage({type:'error',text:err.message})}}
  ...
  return <><section className="card"><div className="card-heading"><div><p className="eyebrow">CONFIGURACIÓN DE ACCESOS</p><h2>Registrar usuario</h2></div></div><p className="muted">El sistema generará una contraseña temporal y la enviará al correo. El usuario deberá cambiarla al iniciar sesión.</p><form className="form-grid" onSubmit={create}><label>Nombre<input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required /></label><label>Correo<input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required /></label><label>Perfil inicial<select value={form.role} onChange={e=>setForm({...form,role:e.target.value})}>{roles.map(x=><option key={x[0]} value={x[0]}>{x[1]}</option>)}</select></label><div className="full form-actions">{message&&<div className={`notice ${message.type}`}>{message.text}</div>}<button className="primary">Registrar y enviar acceso</button></div></form></section>
...
function App(){
  ...
  const titles={expenses:'Solicitudes de gasto del PH',invoices:'Consulta de facturas',users:'Configuración de permisos',categories:'Categorías y subcategorías'}
  return <><header className="topbar"><div><div className="brand-mark">PH</div><div><strong>Gestión de Gastos</strong><span>{user.name} · {roleName(user.role)}</span></div></div><div className="header-actions"><button onClick={()=>setTab('expenses')}>Solicitudes</button>{canView&&<button onClick={()=>setTab('invoices')}>Facturas</button>}{canConfigure&&<><button onClick={()=>setTab('users')}>Permisos</button><button onClick={()=>setTab('categories')}>Categorías</button></>}<button onClick={logout}>Salir</button></div></header><main className="layout"><section className="hero"><p className="eyebrow">CONTROL · TRAZABILIDAD · APROBACIÓN</p><h1>{titles[tab]}</h1></section>{tab==='invoices'&&canView?<Invoices categoryOptions={categoryOptions}/>:tab==='users'&&canConfigure?<Users/>:tab==='categories'&&canConfigure?<CategorySettings onChanged={()=>setRefresh(x=>x+1)}/>:<>{canCreate&&<ExpenseForm onCreated={created} draft={revision} onCancelEdit={()=>setRevision(null)} categoryOptions={categoryOptions} subcategoryOptions={subcategoryOptions}/>}<ExpenseTable refreshKey={refresh} canEdit={canCreate} isAdmin={user.role==='ADMIN'} onEdit={startRevision} onChanged={()=>setRefresh(x=>x+1)} categoryOptions={categoryOptions}/></>}</main></>
}
